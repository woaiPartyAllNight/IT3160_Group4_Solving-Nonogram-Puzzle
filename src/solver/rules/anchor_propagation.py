...
def apply_rule3_3_first_block_col(board, bound, j, k, col_list, previous_end, forward_start, m, n):
    ...
    if board[bound[1][j][k][0]][j] == 1:
        # 1. Tô đen toàn bộ khối từ đầu đến cuối
        ...
        # 2. Cập nhật giới hạn dưới của khối
        ...
        # ========================================================
        # [SỬA LỖI]: ĐÁNH DẤU X (-1) VÀO Ô NGAY SAU KHỐI ĐỂ NGĂN CÁCH
        # ========================================================
        end_idx = bound[1][j][k][0] + col_list[j][k]
        if end_idx < m: # Đảm bảo không tràn viền dưới
        ...
        # 4. Đẩy giới hạn của khối tiếp theo (k+1) xuống
...
def apply_rule3_3_last_block_row(board, bound, i, k, row_list, previous_end, forward_start, m, n):
    ...
    if board[i][bound[0][i][k][1]] == 1:
        # 1. Tô đen toàn bộ khối từ cuối lên đầu
        ...
        # 2. Cập nhật giới hạn dưới của khối k
        ...
        # ========================================================
        # [SỬA LỖI]: ĐÁNH DẤU X (-1) VÀO Ô NGAY TRƯỚC KHỐI ĐỂ NGĂN CÁCH
        # ========================================================
        start_idx = bound[0][i][k][1] - row_list[i][k]
        if start_idx >= 0: # Đảm bảo không tràn viền trái
        ...
        # 4. Đẩy giới hạn của khối phía trước (k-1) lùi lại
        if k > 0:
        ...
        # Cập nhật giới hạn khối k+1 (đã có trong code cũ)
...
def apply_rule3_3_first_block_row(board, bound, i, k, row_list, previous_end, forward_start, m, n):
    ...
    if board[i][bound[0][i][k][0]] == 1:
        ...
        # 2. Cập nhật lại Bound của khối hiện tại
        ...
        # ========================================================
        # [SỬA LỖI] 3. ĐÁNH DẤU X (-1) VÀO Ô NGAY SAU KHỐI ĐỂ NGĂN CÁCH
        # ========================================================
        ...
        # 4. Đẩy Bound của khối tiếp theo (k+1) ra xa
...
def apply_rule3_3_last_block_col(board, bound, j, k, col_list, previous_end, forward_start, m, n):
    ...
    if board[bound[1][j][k][1]][j] == 1:
        ...
        # 4. Đẩy giới hạn của khối phía trước (k-1) lên trên
        if k > 0:
