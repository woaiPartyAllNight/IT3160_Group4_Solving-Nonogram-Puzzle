...
from solver.utils import display_board
from IPython.display import clear_output


def logical(board, bound, row_list, col_list, m, n, visualize=False):
    change = True
    while change:
        change = False
        ...
        for i in range(m):
            for k in range(len(row_list[i])):
                ...
                for func in [apply_rule2_1_block_row, apply_rule2_2_block_row,
                    ...
                    if not valid:
                        print(f"Failed at {func.__name__} for row {i} block {k}")
                        return False
                    if ch:
                ...
                if bound[0][i][k][0] > previous_end:
                    ...
                    if not valid:
                        print(f"Failed at apply_rule3_3_first_block_row for row {i} block {k}")
                        return False
                    if ch:
                        if visualize: display_board(board)
                    change |= ch
                ...
                if bound[0][i][k][1] < forward_start:
                    ...
                    if not valid:
                        print(f"Failed at apply_rule3_3_last_block_row for row {i} block {k}")
                        return False
                    if ch:
                        if visualize: display_board(board)
                    change |= ch
        ...
        for j in range(n):
            for k in range(len(col_list[j])):
                ...
                for func in [apply_rule2_1_block_col, apply_rule2_2_block_col,
                    ...
                    if not valid:
                        print(f"Failed at {func.__name__} for col {j} block {k}")
                        return False
                    if ch:
                        if visualize: display_board(board)
                    change |= ch
                ...
                if bound[1][j][k][0] > previous_end:
                    ...
                    if not valid:
                        print(f"Failed at apply_rule3_3_first_block_col for col {j} block {k}")
                        return False
                    if ch:
                        if visualize: display_board(board)
                    change |= ch
                ...
                if bound[1][j][k][1] < forward_start:
                    ...
                    if not valid:
                        print(f"Failed at apply_rule3_3_last_block_col for col {j} block {k}")
                        return False
                    if ch:
                        if visualize: display_board(board)
                    change |= ch
