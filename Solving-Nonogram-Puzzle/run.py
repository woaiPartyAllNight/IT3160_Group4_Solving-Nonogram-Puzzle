...
# -----------------------------------------------------------------------------
# HIỂN THỊ BẢNG (pygame) - responsive: tự scale theo kích thước cửa sổ
#   1  = ô đen (filled)
#  -1  = ô trắng với dấu chấm (blank)
#   0  = ô xám nhạt (unknown)
# -----------------------------------------------------------------------------

# Màu sắc (không đổi khi resize)
C_BG         = (255, 255, 255)
C_UNKNOWN    = (220, 220, 220)
C_BLACK      = (30,  30,  30)
C_BLANK_BG   = (255, 255, 255)
C_DOT        = (180, 180, 180)
C_LINE_THIN  = (160, 160, 160)
C_LINE_THICK = (30,  30,  30)
C_CLUE_BG    = (245, 245, 245)
C_TEXT       = (0,   0,   0)
C_BTN        = (200, 200, 200)

PAD          = 10   # padding cố định ngoài cùng (px)
BORDER_THIN  = 1
BORDER_THICK = 3
BTN_H        = 36   # chiều cao thanh nút cố định (px)
BTN_GAP      = 8    # khoảng cách giữa thanh nút và lưới

screen  = None
_font_cache = {}   # cache font theo size để tránh tạo lại liên tục
_prev_board = None
paused  = False

# Kích thước cửa sổ hiện tại (cập nhật khi resize)
_win_w = 0
_win_h = 0
...
def _get_font(size, bold=False):
    ...
    if key not in _font_cache:
        _font_cache[key] = pygame.font.SysFont('Arial', max(8, size), bold=bold)
    return _font_cache[key]


def _compute_layout():
    """
    Tính toàn bộ tham số layout từ kích thước cửa sổ hiện tại.
    Trả về dict chứa cell_size, top_margin, left_margin, grid_x0, grid_y0,
    grid_w, grid_h, btn_rects.
    """
    rows = m
    cols = n
    max_col_rows = max(len(c) for c in col_list) if col_list else 1
    max_row_cols = max(len(r) for r in row_list) if row_list else 1

    # Vùng khả dụng cho lưới (sau khi trừ padding, gợi ý, và thanh nút)
    avail_w = _win_w - 2 * PAD - BORDER_THICK
    avail_h = _win_h - 2 * PAD - BORDER_THICK - BTN_H - BTN_GAP

    # cell_size phải thoả mãn cả trục x lẫn y
    # Trục x: (max_row_cols + cols) * cell_size + (cols+1)*BORDER_THIN <= avail_w
    # Trục y: (max_col_rows + rows) * cell_size + (rows+1)*BORDER_THIN <= avail_h
    cs_x = max(8, (avail_w - (cols + 1) * BORDER_THIN) // (max_row_cols + cols))
    cs_y = max(8, (avail_h - (rows + 1) * BORDER_THIN) // (max_col_rows + rows))
    cs = min(cs_x, cs_y)

    left_margin = PAD + max_row_cols * cs + BORDER_THICK
    top_margin  = PAD + max_col_rows * cs + BORDER_THICK

    grid_w = cols * cs + (cols + 1) * BORDER_THIN
    grid_h = rows * cs + (rows + 1) * BORDER_THIN

    # Thanh nút đặt ngay dưới lưới, căn giữa
    btn_y    = top_margin + grid_h + BTN_GAP
    btn_w    = 90
    total_btn_w = 3 * btn_w + 2 * 16
    btn_x0   = (left_margin + grid_w - total_btn_w) // 2 + PAD // 2
    pause_rect  = pygame.Rect(btn_x0,              btn_y, btn_w, BTN_H)
    resume_rect = pygame.Rect(btn_x0 + btn_w + 16, btn_y, btn_w, BTN_H)
    close_rect  = pygame.Rect(btn_x0 + 2*(btn_w+16), btn_y, btn_w, BTN_H)
    ...
    return {
        ...
        'rows': rows,
        'cols': cols,
        'max_col_rows': max_col_rows,
        'max_row_cols': max_row_cols,
...
def init_display(m_rows, n_cols):
    global screen, _prev_board, _win_w, _win_h

    pygame.init()
    pygame.font.init()

    # Set kích thước cửa sổ ban đầu tương đương terminal 80x25 (640x400 px)
    _win_w = 140 * 8  # 640 px
    _win_h = 50 * 16  # 400 px

    screen = pygame.display.set_mode((_win_w, _win_h), pygame.RESIZABLE)
    pygame.display.set_caption('Nonogram Solver')
    _prev_board = None


def _draw_full(board):
    """Vẽ lại toàn bộ màn hình dựa trên kích thước cửa sổ hiện tại."""
    L = _compute_layout()
    cs      = L['cs']
    left    = L['left']
    top     = L['top']
    ...
    font_clue = _get_font(max(8, int(cs * 0.46)), bold=True)
    font_btn  = _get_font(max(8, int(BTN_H * 0.48)))

    screen.fill(C_BG)

    # 1. Nền vùng gợi ý
    pygame.draw.rect(screen, C_CLUE_BG,
                     (left, PAD, grid_w, top - PAD))           # cột (trên)
    pygame.draw.rect(screen, C_CLUE_BG,
                     (PAD, top, left - PAD, grid_h))            # hàng (trái)

    # 2. Gợi ý cột
    for j, clues in enumerate(col_list):
        cx = left + BORDER_THIN + j * (cs + BORDER_THIN) + cs // 2
        for idx, val in enumerate(reversed(clues)):
            cy = top - (idx + 1) * cs + cs // 2
            surf = font_clue.render(str(val), True, C_TEXT)
            screen.blit(surf, surf.get_rect(center=(cx, cy)))

    # 3. Gợi ý hàng
    for i, clues in enumerate(row_list):
        cy = top + BORDER_THIN + i * (cs + BORDER_THIN) + cs // 2
        for idx, val in enumerate(reversed(clues)):
            cx = left - (idx + 1) * cs + cs // 2
            surf = font_clue.render(str(val), True, C_TEXT)
            screen.blit(surf, surf.get_rect(center=(cx, cy)))
    ...
    for i in range(rows):
        for j in range(cols):
            x = left + BORDER_THIN + j * (cs + BORDER_THIN)
            y = top  + BORDER_THIN + i * (cs + BORDER_THIN)
            v = board[i][j]
            if v == 1:
                pygame.draw.rect(screen, C_BLACK, (x, y, cs, cs))
            elif v == -1:
                pygame.draw.rect(screen, C_BLANK_BG, (x, y, cs, cs))
                r = max(2, cs // 14)
                pygame.draw.circle(screen, C_DOT, (x + cs // 2, y + cs // 2), r)
            else:
                pygame.draw.rect(screen, C_UNKNOWN, (x, y, cs, cs))

    # 5. Đường kẻ lưới mỏng
    for i in range(rows + 1):
        yy = top + i * (cs + BORDER_THIN)
        pygame.draw.line(screen, C_LINE_THIN, (left, yy), (left + grid_w, yy), BORDER_THIN)
    for j in range(cols + 1):
        xx = left + j * (cs + BORDER_THIN)
        pygame.draw.line(screen, C_LINE_THIN, (xx, top), (xx, top + grid_h), BORDER_THIN)

    # 6. Đường kẻ đậm phân cách
    pygame.draw.line(screen, C_LINE_THICK, (PAD, top),  (left + grid_w, top),  BORDER_THICK)
    pygame.draw.line(screen, C_LINE_THICK, (left, PAD), (left, top + grid_h),  BORDER_THICK)

    # 7. Nút Pause / Resume / Close
    for rect, label in [
        (L['pause_rect'],  'Pause'),
        (L['resume_rect'], 'Resume'),
        (L['close_rect'],  'Close'),
    ]:
        pygame.draw.rect(screen, C_BTN, rect, border_radius=4)
        pygame.draw.rect(screen, C_LINE_THICK, rect, 2, border_radius=4)
        surf = font_btn.render(label, True, C_TEXT)
...
def display_board(board):
    """Xử lý sự kiện pygame và render bảng (chỉ flip khi cần)."""
    global _prev_board, paused, _win_w, _win_h, screen
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            # Cập nhật kích thước và vẽ lại — layout tự tính theo _win_w/_win_h
            _win_w, _win_h = event.w, event.h
            screen = pygame.display.set_mode((_win_w, _win_h), pygame.RESIZABLE)
            _draw_full(board)
            _prev_board = None  # force redraw
        elif event.type == MOUSEBUTTONDOWN:
            L = _compute_layout()
            if L['pause_rect'].collidepoint(event.pos):
                paused = True
                _draw_full(board)
            elif L['resume_rect'].collidepoint(event.pos):
                paused = False
                _draw_full(board)
            elif L['close_rect'].collidepoint(event.pos):
                pygame.quit()
                sys.exit()

    if board != _prev_board:
        _draw_full(board)
        _prev_board = [row[:] for row in board]
...
# Khởi tạo cửa sổ pygame dựa trên kích thước lưới
init_display(m, n)
...
# Hiển thị trạng thái cuối cùng và chờ người dùng đóng cửa sổ
_draw_full(board)
running = True
...
while running:
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            _win_w, _win_h = event.w, event.h
            screen = pygame.display.set_mode((_win_w, _win_h), pygame.RESIZABLE)
            _draw_full(board)   # tính lại layout và vẽ lại
        elif event.type == MOUSEBUTTONDOWN:
            L = _compute_layout()
            if L['close_rect'].collidepoint(event.pos):
                running = False
            elif L['pause_rect'].collidepoint(event.pos):
                paused = True
                _draw_full(board)
            elif L['resume_rect'].collidepoint(event.pos):
                paused = False
